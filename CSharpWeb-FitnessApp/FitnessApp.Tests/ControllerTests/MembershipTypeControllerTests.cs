...
public class MembershipTypeControllerTests
{
    ...
    [Test]
    public async Task Details_RedirectsToIndex_WhenMembershipTypeDetailsIsNull()
    {
        // Arrange
        var mockService = new Mock<IMembershipTypeService>();
        var membershipTypeId = 1;
        mockService.Setup(service => service.GetMembershipTypeDetailsAsync(membershipTypeId))
            .ReturnsAsync((MembershipTypeDetailsViewModel)null);
        var controller = new MembershipTypeController(mockService.Object);
    ...
    [Test]
    public async Task AddMyMembership_RedirectsToMyMembershipType_WhenSuccessful()
    {
        // Arrange
        var mockService = new Mock<IMembershipTypeService>();
        var userId = "testUserId";
        ...
        mockService.Setup(service => service.AddMyMembershipAsync(userId, membershipType))
        ...
        // Act
        var result = await controller.AddMyMembership(membershipTypeId);

        // Assert
        Assert.IsInstanceOf<RedirectToActionResult>(result);
        var redirectResult = result as RedirectToActionResult;
        Assert.NotNull(redirectResult);
        Assert.AreEqual(nameof(controller.MyMembershipType), redirectResult.ActionName);
    }

    [Test]
    public async Task AddMyMembership_RedirectsToDetails_WhenMembershipTypeDoesNotExist()
    {
        // Arrange
        var mockService = new Mock<IMembershipTypeService>();
        var membershipTypeId = 1;
        mockService.Setup(service => service.GetMembershipTypeByIdAsync(membershipTypeId))
            .ReturnsAsync((MembershipTypeViewModel)null);
        var controller = new MembershipTypeController(mockService.Object);
        ...
        var redirectResult = result as RedirectToActionResult;
        Assert.NotNull(redirectResult);
        Assert.AreEqual(nameof(controller.Details), redirectResult.ActionName);
        Assert.AreEqual(membershipTypeId, redirectResult.RouteValues["id"]);
    }

    [Test]
    public async Task RemoveMyMembership_RedirectsToMyMembershipType_WhenSuccessful()
    {
        // Arrange
        var mockService = new Mock<IMembershipTypeService>();
        var userId = "testUserId";
